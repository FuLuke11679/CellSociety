...
public class SimulationController {
  ...
  public void displaySplashScreen(Scene splashScene){
    splashLoop = new Timeline(new KeyFrame(Duration.seconds(0.05), e -> {
      //myLocale = mySplashScreen.getMyLocale(); //update locale if user switches language (done in splashscreen class with setter)
      //myScheme = splashScreen.getColorScheme();  //update color scheme per user input (if works, do the same as with locale above)
  ...
  private BorderPane initializeLayout(ResourceBundle simInfo) {
    ...
    pauseButton.setOnAction(e -> {
      if (simLoop != null) {
        ...
        else if (simLoop.getStatus() == Animation.Status.PAUSED) {  //REMOVE?
    ...
    loadButton.setOnAction(e -> {
      File newFile = FILE_CHOOSER.showOpenDialog(myStage);
      if (newFile != null) {
