...
/**
 * Factory class for creating instances of {@link CellShape}. This class dynamically loads a
 * specific {@code CellShape} implementation based on the provided shape type. If the specified
 * shape type is null, empty, or if any error occurs during instantiation, it defaults to returning
 * a {@link RectangularShape}.
 *
 * @author Luke
 */
public class CellShapeFactory {
   ...
   * "cellsociety.model.grid.shape.&lt;shapeType&gt;Shape" using reflection. If the shape type is
   * invalid or any error occurs, a {@link RectangularShape} is returned as a fallback.
   * </p>
   *
   * @param shapeType the type of cell shape to create (e.g., "Rectangular", "Hexagonal")
   * @return an instance of the requested {@code CellShape}, or a default {@code RectangularShape}
   * if creation fails
   */
